    ...
    async performCppAnalysis(text) {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                const response = await axios_1.default.post(`${this.cppServiceUrl}/analyze`, requestPayload, axiosConfig);
        ...
        if (lastError?.code === 'ECONNREFUSED') {
            ...
            console.error(`[C++ Service] 📡 HTTP ${lastError?.response?.status}: ${lastError?.response?.statusText}`);
            console.error(`[C++ Service] 📄 Response body:`, lastError?.response?.data);
        }
